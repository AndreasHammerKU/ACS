...
public class CertainBookStore implements BookStore, StockManager {
	...
	public synchronized List<Book> getTopRatedBooks(int numBooks) throws BookStoreException {
		...
		List<BookStoreBook> stockBooks = bookMap.entrySet().stream().map(pair -> pair.getValue())
				.collect(Collectors.toList());
		Set<Integer> tobePicked = new HashSet<>();
		int rangePicks = stockBooks.size();

		SortedMap<Float, BookStoreBook> bookAndRatings = new TreeMap<>();

		for (BookStoreBook book : stockBooks) {
			float avgRating = book.getAverageRating();
			bookAndRatings.put(avgRating, book);
		}

		List<BookStoreBook> sortedBooks = bookAndRatings.entrySet().stream().map(pair -> pair.getValue()).
				collect(Collectors.toList());
		...
		return tobePicked.stream().map(index -> sortedBooks.get(index).immutableBook())
				.collect(Collectors.toList());
	}
